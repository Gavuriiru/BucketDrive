...
export const Errors = {
  ...
  forbidden: (msg = "Permission denied") => new AppError("FORBIDDEN", msg, 403),
  ...
  quotaExceeded: (msg = "Storage quota exceeded") => new AppError("QUOTA_EXCEEDED", msg, 413),
...
app.post("/api/workspaces/:id/files/upload", async (c) => {
  ...
  if (used + sizeBytes > quotaBytes) {
    throw Errors.quotaExceeded(`Workspace quota exceeded. Used: ${used}, Limit: ${quotaBytes}`)
  }
  // ...
})
```
