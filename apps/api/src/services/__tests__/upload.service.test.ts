...
describe("buildUploadStorageKey", () => {
  it("preserves the uploaded file name and extension in the R2 key", () => {
    expect(buildUploadStorageKey("upload-1", "AGENTS.md")).toBe("bucket/files/upload-1/AGENTS.md")
  })

  it("uses the basename when a browser provides a path-like name", () => {
    expect(buildUploadStorageKey("upload-1", "folder/Notes Final.pdf")).toBe(
      "bucket/files/upload-1/Notes Final.pdf",
    )
  })
})
